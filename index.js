...
const extensionName = "st-text-cleaner-settings";
...
function cleanText(text) {
    ...
    for (let i = 0; i < parts.length; i++) {
        // ถ้าส่วนนี้ไม่ได้ขึ้นต้นด้วย <think> ให้ลบจุดออกทั้งหมด (ลบให้ทั้งในและนอก HTML)
    ...
    try {
        ...
        if (msg && !msg.is_user) {
            ...
            if (originalText !== cleanedText) {
                ...
                // ใช้ Dynamic Import เพื่อป้องกันบัคกรณี SillyTavern อัปเดตโครงสร้างไฟล์
                const chatModule = await import("../../../../chat.js");
                if (chatModule.saveChat) chatModule.saveChat();
                if (chatModule.updateMessageBlock) chatModule.updateMessageBlock(messageId, msg);
            }
        }
    } catch (err) {
        console.error("[Text Cleaner] Error parsing message:", err);
    }
}

// รอให้ระบบโหลด UI เสร็จก่อนแล้วค่อยแทรกเมนู
jQuery(async () => {
    try {
        ...
        // ยัด HTML ใส่หน้าต่างตั้งค่า Extensions
        ...
        console.error("[Text Cleaner] Failed to load UI:", err);
    }
});